...
| `on-demand` *(default)* | Cache file chunks and metadata on access; revalidate and re-fetch when the server copy changes. If the server becomes unreachable, already-cached chunks and listings keep being served. |
...
Content is cached in `4 MiB` chunks by default. Use `--cache-chunk-size SIZE`
to change it; plain bytes and `K`/`M`/`G` suffixes are accepted, e.g. `8M`.
